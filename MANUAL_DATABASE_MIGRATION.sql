-- ============================================================================
-- CalorieWise Database Migration - Complete SQL Script
-- Run this manually in Supabase Dashboard → SQL Editor
-- ============================================================================

-- PART 1: Add Streak Shield System
-- ============================================================================
...
-- RLS Policies
DROP POLICY IF EXISTS "Milestones are viewable by everyone" ON public.streak_milestones;
CREATE POLICY "Milestones are viewable by everyone"
  ON public.streak_milestones FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can view own rewards" ON public.streak_rewards;
CREATE POLICY "Users can view own rewards"
  ON public.streak_rewards FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own rewards" ON public.streak_rewards;
CREATE POLICY "Users can insert own rewards"
  ON public.streak_rewards FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- ============================================================================
-- PART 2: Add Pro Access Tracking
-- ============================================================================

-- Add pro access tracking to profiles
ALTER TABLE public.profiles 
...
-- 1. Check new columns exist in profiles
-- SELECT column_name, data_type 
-- FROM information_schema.columns 
-- WHERE table_name = 'profiles' 
-- AND column_name IN ('streak_shields', 'longest_streak_days', 'last_streak_check_date', 'pro_access_until');

-- 2. Check streak_milestones table exists and has data
-- SELECT * FROM public.streak_milestones ORDER BY days_required;

-- 3. Check streak_rewards table exists
-- SELECT COUNT(*) FROM public.streak_rewards;

-- 4. Check trigger exists
-- SELECT trigger_name FROM information_schema.triggers 
-- WHERE trigger_name = 'update_user_streak_trigger';
...
-- DROP TRIGGER IF EXISTS update_user_streak_trigger ON public.profiles;
-- DROP FUNCTION IF EXISTS trigger_update_user_streak();
-- DROP TABLE IF EXISTS public.streak_rewards;
-- DROP TABLE IF EXISTS public.streak_milestones;
-- ALTER TABLE public.profiles DROP COLUMN IF EXISTS streak_shields;
-- ALTER TABLE public.profiles DROP COLUMN IF EXISTS longest_streak_days;
-- ALTER TABLE public.profiles DROP COLUMN IF EXISTS last_streak_check_date;
-- ALTER TABLE public.profiles DROP COLUMN IF EXISTS pro_access_until;
-- DROP INDEX IF EXISTS idx_profiles_pro_access;
